...
export async function POST(request: NextRequest) {
  ...
  const { error: sendError } = await resend.emails.send({
    ...
    to: email,
    subject: 'Confirme seu e-mail — Finxa',
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#fff">
        <h2 style="color:#18181b;margin-bottom:8px">Acesse o Finxa</h2>
        <p style="color:#52525b;margin-bottom:24px">
          Clique no botão abaixo para confirmar seu e-mail e entrar na sua conta diretamente.
        </p>
        ...
        <p style="color:#a1a1aa;font-size:12px;margin-top:32px">
          Se você não criou uma conta no Finxa, ignore este e-mail.
        </p>
      </div>
    `,
  })
